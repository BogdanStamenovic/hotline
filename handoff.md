...
>
> - **Whisper's 4.78 s load is paid in the middle of his first sentence.**
>   `Ears.load()` is called from `transcribe()`, so the first call after any
>   restart eats it while he is talking. His instruction is on-demand loading;
>   the detail that makes it work is **load on RING, not on ANSWER** — the ring
>   is 45 s and both models together are under ten. `CallAgent.start()` already
>   uses that pattern. Neither model ever unloads today.
